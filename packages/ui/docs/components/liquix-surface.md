...
Shapes register with the surface through `useLiquixBox(layer)`. It hands back a ref to hang on a transparent element, and a mutable entry whose shape the frame loop reads every frame, so a caller can animate a box for nothing. The entry's `alpha` fades the shape's glass over the layer below, which is how a shape can cross-fade with something else rather than be cut. Its `clarity` scales the rim, the refraction and the dispersion of the shape's pass and frosts the body as it falls, so glass can dissolve rather than switch off, and its `shadow` asks for a drop shadow, drawn as alpha outside the glass so it can sit under a lifted shape. [LiquixTabs](/docs/components/liquix-tabs) is built on it.

The shader merges every shape in a pass into one distance field with `min()`, so a pill inside a bar would be swallowed by it: inside the bar, the bar is always the deeper shape. Shapes are therefore grouped by `layer` and drawn in separate passes, each composited over the ones before it. That is how a highlight comes to sit on a bar's glass rather than merge into it. Every layer refracts the content itself, not the glass below: a lens looking at a lens would show the lower shape's rim inside it. At most six shapes are drawn per layer.

## Defaults, and what the shader gives up here

`defaultLiquixSurfaceParams` is `defaultLiquixParams` tuned for a bar over live content rather than a capsule over a backdrop, the way Apple's Liquid Glass behaves: the body of a pane is a lightly blurred (`blurRadius` 6), darkly tinted (45% of a near-black) view of what is behind it, and the bending lives in a 10px rim (`refThickness`), where the backdrop is sharp at the lip (`blurEdge` off), the chromatic fringe is faint (`refDispersion` 3) and the specular highlights ride. A shape that wants a deeper lens asks for it through its entry's `bevel`; the pass takes the deepest asked for. The body dims further over bright content (`overLight` 50), which together with the dark tint keeps white labels legible over any picture, a pale sky as much as dark trees. Two things are turned off:

- The drop shadow, by default. On a surface it is drawn as alpha outside the glass, per shape, rather than baked into the backdrop where it would survive only as a dirty ring inside the rim. A shape asks for one through its entry's `shadow`; for a shape that never moves, a CSS shadow in `underlay` is cheaper.
